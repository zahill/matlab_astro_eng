...
[V_ball,h_ball,t_ball] = ballistic_entry_phase(entryVel,entryAlt,h_open_chute,FPA,Cd_capsule,d_capsule,entryMass);
[V_chute,h_chute,t_chute] = parachute_phase(h_open_chute,V_ball(1),d_capsule,Cd_capsule,entryMass,number_of_chutes,cluster_performance,d_chute,theta_trim,Cd_chute);
...
ylabel("Velocity [m/s]")
xlabel("Altitude [km]")
plot(h_ball/1000,V_ball,"LineWidth",2)
hold on
plot(h_chute/1000,V_chute,"LineWidth",2)
formatPlot
xlim([0 entryAlt/1000-600])
legend("Ballistic Entry Phase","Parachute Phase","Location", "northwest")
fprintf("Duration of ballistic phase = %.2f min\n",t_ball(end)/(60))
fprintf("Duration of parachute phase = %.2f min\n",t_chute(end)/(60))

calculate_downrange_dist(flip(h_ball),h_chute,FPA,theta_trim);

function [V,h,t] = parachute_phase(h_open_chute,v_initial,d_capsule,Cd_capsule,entryMass,number_of_chutes,cluster_performance,d_chute,theta_trim,Cd_chute)
...
function [V,h,t,N_max,Hn_max] = ballistic_entry_phase(entryVel,entryAlt,h_open_chute,FPA,Cd,d_capsule,entryMass)
...
function [S, fig] = calculate_downrange_dist(h_ball,h_chute,FPA,theta_trim)
% CALCULATE_DOWNRANGE Calculate and plot the downrange distance given the
% altitude, FPA, and trim_angle.
%
% IMPORTANT: The altitude vectors must be in the correct time order 
% 
% INPUTS:
%     - h_ball    : The ballistic entry altitude vector.
%     - h_chute   : The parachute entry altitude vector.
%     - FPA       : The flight path angle during the ballistic entry.
%     - theta_trim: The trim angle during the parachute phase of flight.
%
% OUTPUTS:
%     - S         : The total downrange distance vector
%     - fig       : The figure for the downrange distance vs. altitude plot
